...
/// @title Implement operational logic for CreditVault
/// @author Native
/// @notice Extract operational logic from CreditVault to reduce contract size. The operation would be delegatecall from CreditVault
library CreditVaultLogic {
  ...
  event EpochUpdate(
    ICreditVault.TraderPositionUpdate[] traderPositionUpdate,
    ICreditVault.LpTokenValueUpdate[] lpTokenValueUpdate
  );
  event Repaid(address trader, ICreditVault.TokenAmountInt[] repayments);
  event Settled(address trader, ICreditVault.TokenAmountInt[] positionUpdates);
  event CollateralAdded(
    address trader,
    ICreditVault.TokenAmountUint[] collateralUpdates
  );
  ...
  event Liquidated(
    address trader,
    address liquidator,
    ICreditVault.TokenAmountInt[] positionUpdates,
    ICreditVault.TokenAmountUint[] claimCollaterals
  );
  ...
  function swapCallback(
    ...
    address buyerToken,
    int256 amountOut,
    mapping(address => CreditLpToken) storage lpTokens,
  ...
  function pay(address to, uint256 amount) external {
    IERC20(CreditLpToken(msg.sender).underlying()).safeTransfer(to, amount);
  }
  ...
  function setAllowance(
    ICreditVault.TokenAmountUint[] calldata tokens,
    address nativePool
  ) external {
    for (uint256 i; i < tokens.length; ) {
      if (tokens[i].tokenAddress == address(0)) revert ICreditVault.ZeroError();
  ...
  /// @dev The function is called by CreditVault to update positions and borrow values in CreditLpToken
  ...
  /// @param positions mapping in storage passed by CreditVault for trader address to token address to position amount
  /// @param lpTokens mapping in storage passed by CreditVault for token address to CreditLpToken
  function positionEpochUpdate(
    ICreditVault.TraderPositionUpdate[] calldata traderPositionUpdate,
    ICreditVault.LpTokenValueUpdate[] calldata lpTokenValueUpdate,
    mapping(address => mapping(address => int256)) storage positions,
    mapping(address => CreditLpToken) storage lpTokens,
    uint256 lastEpochUpdateTimestamp
  ) external {
    if (
      block.timestamp - lastEpochUpdateTimestamp <
      EPOCH_UPDATE_COOL_DOWN_SECONDS
    ) {
    ...
    for (uint i = 0; i < traderPositionUpdate.length; ) {
      ...
      for (
        uint j = 0;
        j < traderPositionUpdate[i].tokenAmountUpdates.length;

      ) {
        address token = traderPositionUpdate[i]
          .tokenAmountUpdates[j]
          .tokenAddress;
        ...
        positions[trader][token] += traderPositionUpdate[i]
          .tokenAmountUpdates[j]
          .amount;
        unchecked {
          j++;
    ...
    for (uint i = 0; i < lpTokenValueUpdate.length; ) {
      ...
      lpTokens[token].updateNetBorrow(
        lpTokenValueUpdate[i].netBorrowChange.toInt256()
      );
      ...
      if (
        ...
        ((newExchangeRate - lpExchangeRateBeforeUpdate) * 10_000) /
          lpExchangeRateBeforeUpdate >
  ...
  function repay(
    ICreditVault.TokenAmountInt[] calldata repayments,
  ...
  /// @param lpTokens mapping in storage passed by CreditVault for token address to CreditLpToken
  function settle(
    ICreditVault.TokenAmountInt[] memory positionUpdates,
  ...
  /// @param aquaCollateral mapping in storage passed by CreditVault for trader address to token address to collateral amount
  /// @param markets mapping in storage passed by CreditVault for token address to Compound markets (LP token status)
  ...
  function liquidate(
    ICreditVault.TokenAmountInt[] memory positionUpdates,
    ICreditVault.TokenAmountUint[] calldata claimCollaterals,
    TraderCreditCheckParams memory params,
    mapping(address => mapping(address => int256)) storage positions,
    mapping(address => CreditLpToken) storage lpTokens,
    mapping(address => ComptrollerV2Storage.Market) storage markets,
    ...
    for (uint256 i; i < claimCollaterals.length; ) {
      aquaCollateral[params.trader][
        claimCollaterals[i].tokenAddress
      ] -= claimCollaterals[i].amount;
      unchecked {
        ++i;
      }
    }
  ...
  function updatePositions(
    ...
    for (uint256 i; i < positionUpdates.length; ) {
      ...
      if (
        positions[trader][token] * amount >= 0 ||
        positions[trader][token] * newPositionValue < 0
      ) {
  ...
  function getCredit(
    ...
    for (uint256 j; j < params.allMarkets.length; ) {
      ...
      address token = address(CreditLpToken(lpToken).underlying());
          ...
          ((
            positions[params.trader][token] > 0
              ? (positions[params.trader][token] *
                (markets[lpToken].collateralFactorMantissa).toInt256()) /
      ...
      if (aquaCollateral[params.trader][lpToken] > 0) {
        credit += ((((aquaCollateral[params.trader][lpToken] *
          markets[lpToken].collateralFactorMantissa) / ETHER_SCALE) * price) /
          ETHER_SCALE).toInt256();
      }
      unchecked {
        ++j;
