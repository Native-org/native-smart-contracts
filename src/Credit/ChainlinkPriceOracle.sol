...
/// @title Chainlink price oracle for Native Credit
/// @author Native
/// @notice The oracle is used to get the price of a token in USD and determine the borrow and liquidation threshold
/// @notice Native Credit only sets the price feed for the tokens that are open for retail users to borrow
contract ChainlinkPriceOracle is PriceOracle {
  ...
  function getUnderlyingPrice(
    ...
    uint256 chainlinkDecimalsDelta = EXPECTED_DECIMALS -
      uint256(feed.decimals());

    uint256 chainlinkPrice = price.toUint256() * 10 ** chainlinkDecimalsDelta;

    uint8 tokenDecimals = IERC20Decimal(
